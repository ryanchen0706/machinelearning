...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        one = nn.ReLU(nn.AddBias(nn.Linear(x, self.w1), self.b1))
        two = nn.ReLU(nn.AddBias(nn.Linear(one, self.w2),self.b2))
        output = nn.AddBias(nn.Linear(two, self.w3), self.b3)
    ...
    def train(self, dataset):
        ...
        while l >= 0.02:
            for x, y in dataset.iterate_once(100):
                l = self.get_loss(x, y)
                print(nn.as_scalar(l))
                grads = nn.gradients(l, self.params)
                l = nn.as_scalar(l)
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        output = nn.AddBias(nn.Linear(third, self.w4), self.b4)
    ...
    def train(self, dataset):
        ...
        while acc < .98:
            for x, y in dataset.iterate_once(100):
                l = self.get_loss(x, y)
                grad = nn.gradients(l, self.params)
                l = nn.as_scalar(l)
                for i in range(len(self.params)):
                    self.params[i].update(grad[i], -self.lr)
